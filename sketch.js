// DOG GENERATOR. Copyright 2020. Richard Banks
// Automatically generates a randomly shaped and coloured dog
...
// Figure out the scaling and number of dogs to draw
function prepareDog() {
  background(100);
  var scaleDogX = width / dogRectX / dogColumns;
  var scaleDogY = height / dogRectY / dogRows;
  if (scaleDogX > scaleDogY) scaleDog = scaleDogY
  else scaleDog = scaleDogX;
  scale(scaleDog);
  for (let j = 0; j < dogRows; j++) {
    for (let i = 0; i < dogColumns; i++) {
      let dogCoatColor = random(dogCoatColorArray);
      createDog(
        (dogRectX / dogColumns / scaleDog) * i,
        topRow / scaleDog + dogRectY * j,
        dogCoatColor
      );
    }
  }
...
function createDog(originX, originY, dogCoatColor) {
  ...
  originX =
    originX * thisDogScaleFactor + (dogRectX - dogRectX / thisDogScaleFactor);
  originY =
    originY * thisDogScaleFactor +
    (dogRectY - dogRectY / thisDogScaleFactor) +
    (topRow / (scaleDog * thisDogScaleFactor) -
      topRow / scaleDog / thisDogScaleFactor);
...
// Figure out if the mouse is over a given region of the screen
